    ...
    in {
      ...
      devShells.default = pkgs.mkShell {
        ...
        shellHook = ''
          ...
          echo "║      Zig 0.15.x | Phase 5.5: GUI         ║"
          echo "╠══════════════════════════════════════════╣"
          echo "║ zig version: $(zig version)              ║"
          echo "╚══════════════════════════════════════════╝"
          echo ""
          echo "Commands:"
          echo "  zig build             Build CLI"
          echo "  zig build -Dgui       Build CLI + GUI"
          echo "  zig build run         Run CLI"
          echo "  zig build run-gui     Run GUI (requires -Dgui)"
      ...
      packages.gui = pkgs.stdenv.mkDerivation {
        ...
        buildPhase = ''
          ...
          export XDG_CACHE_HOME=$TMPDIR/.cache
          mkdir -p $out/bin
          zig build -Doptimize=ReleaseFast -Dgui -Dtarget=x86_64-linux-gnu --prefix $out
        '';

        meta = with pkgs.lib; {
          description = "STL-Next with GUI";
          homepage = "https://github.com/e421/stl-next";
          license = licenses.gpl3;
          platforms = platforms.linux;
          mainProgram = "stl-next";
      ...
      packages.stl-next-gui = self.packages.${system}.gui;
    })
    // {
