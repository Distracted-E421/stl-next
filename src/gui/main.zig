...
const AppState = struct {
    ...
    tinkers: [6]Tinker = .{
        .{ .id = "mangohud", .name = "MangoHud", .key = 'M', .enabled = false, .color = Colors.orange, .tooltip = "Shows FPS, frame time, CPU/GPU usage overlay" },
        .{ .id = "gamescope", .name = "Gamescope", .key = 'G', .enabled = false, .color = Colors.blue, .tooltip = "Valve's compositor for resolution scaling and FSR" },
        .{ .id = "gamemode", .name = "GameMode", .key = 'O', .enabled = true, .color = Colors.green, .tooltip = "Optimizes system for gaming (CPU governor, etc.)" },
        .{ .id = "winetricks", .name = "Winetricks", .key = 'W', .enabled = false, .color = Colors.purple, .tooltip = "Install Windows components (DirectX, .NET, etc.)" },
        .{ .id = "customcmd", .name = "Custom Cmd", .key = 'C', .enabled = false, .color = Colors.cyan, .tooltip = "Run shell commands before/after game launch" },
        .{ .id = "steamgriddb", .name = "SteamGridDB", .key = 'S', .enabled = false, .color = Colors.pink, .tooltip = "Download game artwork (icons, banners, etc.)" },
    },
...
// Helper to get current window height
fn getHeight() i32 {
...
pub fn main() !void {
    ...
    // Initialize window - RESIZABLE for better compatibility
    ray.SetConfigFlags(ray.FLAG_WINDOW_RESIZABLE | ray.FLAG_MSAA_4X_HINT);
    ray.InitWindow(DEFAULT_WIDTH, DEFAULT_HEIGHT, "STL-Next Wait Requester");
    ray.SetTargetFPS(60);
    ...
    ray.SetWindowPosition(
        ...
        @divTrunc(mon_height - DEFAULT_HEIGHT, 2),
    );
...
fn drawTitleBar() void {
    ...
    ray.DrawRectangle(0, 48, w, 2, Colors.surface0);
    ...
    // Close button
    const close_rect = ray.Rectangle{ .x = @as(f32, @floatFromInt(w)) - 45, .y = 10, .width = 35, .height = 30 };
    const close_hover = ray.CheckCollisionPointRec(mouse, close_rect);
    ...
    ray.DrawText("X", w - 35, 13, FONT_MEDIUM, if (close_hover) Colors.base else Colors.subtext);
}
...
fn drawCountdownSection() void {
    ...
    ray.DrawText(&status_buf, getWidth() - 80, y + 10, FONT_TITLE + 10, if (state.paused) Colors.yellow else if (countdown_int <= 3) Colors.red else Colors.green);
    ray.DrawText("sec", getWidth() - 80, y + 55, FONT_TINY, Colors.muted);
    ...
    const bar_rect = ray.Rectangle{
        ...
        .width = @as(f32, @floatFromInt(w)) - 130,
        .height = bar_height,
    };
    ...
    const fill_width = (@as(f32, @floatFromInt(w)) - 138) * progress;
...
fn drawActionsSection() void {
    ...
    const start_x = @divTrunc(getWidth() - total_width, 2);
    const btn_y = y + 20;
...
fn drawStatusBar() void {
    const y = getHeight() - 30;

    // Background
    ray.DrawRectangle(0, y, getWidth(), 30, Colors.mantle);
    ray.DrawRectangle(0, y, getWidth(), 1, Colors.surface0);
    ...
    // Version
    ray.DrawText("STL-Next v0.5.3", getWidth() - 120, y + 8, FONT_TINY, Colors.muted);
}
...
fn drawTooltip() void {
    ...
    if (x + width > getWidth() - 10) x = getWidth() - width - 10;
    if (y + height > getHeight() - 10) y = state.tooltip.y - height - 20;
    if (x < 10) x = 10;
    if (y < 10) y = 10;
